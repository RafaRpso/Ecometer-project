...
select tipoLixeira, dataHoraSensor, round(sum(sinal) / (select count(idLixeira) from lixeira where tipoLixeira = 'Plástico'), 2) from Empresa join kitLixeira on idEmpresa = fkEmpresa join lixeira join sensor on idLixeira = fkLixeira

...
// DEF VAR.
#define DHTPIN A1 //DEFINE PORTAS (ANALOG)
#define LM35PIN A5
#define LUMIPIN A0
#define CHAVPIN 7 //  (DIGITAL)


DHT dht(DHTPIN, DHT11); // SENSOR FUNC. DHT


void setup() // DEFINE O SETUP 
{
	pinMode(DHTPIN, INPUT); // SENSOR DHT UMIDADE
	pinMode(CHAVPIN, INPUT); // input x output
	Serial.begin(9600); //TRANSMISSAO DADO
	dht.begin();
}
void loop() //LOOP ETERNO PARA REPETIR INFINITAMENTE
{
	...
	float luminosidade = analogRead(LUMIPIN); //LE LUMIPIN DE TEMPERATURA
	Serial.print(luminosidade);
	Serial.print("  Temperatura: ");
